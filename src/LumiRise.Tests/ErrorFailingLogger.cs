...
namespace LumiRise.Tests;

/// <summary>
/// Logger that throws <see cref="XunitException"/> when a message at
/// <see cref="LogLevel.Error"/> or <see cref="LogLevel.Critical"/> is logged,
/// causing the current test to fail immediately.
...
public sealed class ErrorFailingLogger<T> : ILogger<T>
{
    ...
    public ErrorFailingLogger(Action<string>? writer = null)
    {
        _writer = writer;
    }
    ...
    public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter)
    {
        ...
        _writer?.Invoke(formatted);
        if (exception != null)
            _writer?.Invoke($"Exception: {exception}");
